...
// Как можно доработать и в чем ошибка выбранного пути? (это по всему коду контракта)

// Контракт не запустится, потому что не понятно как его доработать.
/* Если я деплою контракт и предлагаю кому-то стать участником, то у меня есть адрес контракта который я могу расшарить среди участников.
 А что если еще один человек задеплоит контракт? Будет ли у него вообще возможность деплоить тот же самый контракт?
 В зависимости от ответа станет понятно, нужно ли хранить данные участников и их portions или потребуется какое-то другое хранилище. Например набор байт который будет зашифрован и возвращен вызывающему. И затем расшарен среди участников.
 */
